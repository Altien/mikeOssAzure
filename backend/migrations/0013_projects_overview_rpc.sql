-- 0013_projects_overview_rpc.sql
--
-- Projects overview read model. Returns the visible project list (owned +
-- shared via `shared_with @> [email]`), owner display name, and per-project
-- document/chat/review counts in one call (consumed by `GET /projects` via
-- db.rpc("get_projects_overview") — see backend/src/routes/projects.ts).
--
-- Re-authored in dev's numbered style from upstream's date-based
-- `backend/migrations/20260613_02_projects_overview_rpc.sql` (commit
-- 9a1277ba99cbd7dfae77e5b882e5cef8521fca2f). Plain SQL, no Supabase-auth
-- coupling. Column types line up with dev's schema as-is: projects.user_id
-- is `text` (compared to p_user_id directly) and user_profiles.user_id is
-- `uuid` (cast to text for the join). This replaces dev's prior hand-rolled
-- shared_with query + N+1 count pass — see the divergence comment in
