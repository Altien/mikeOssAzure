// OpenAI **Responses API** adapter (raw fetch to /v1/responses), ported from
// upstream willchen96/mike. One of dev's two OpenAI-direct adapters; the other
// is the Chat Completions adapter in ./openai_c.ts. The dispatcher in ./openai.ts
// selects between them (Completions by default, this one when
// OPENAI_API_MODE=responses). Reconciled with dev's divergences: KV-first secret
// resolution (resolveSecret, internal design notes §2.4) replacing upstream's
// env-var apiKey(); hard-fail on a missing key preserved. abortSignal is plumbed
// through to fetch (unlike the Completions adapter, which still ignores it).
// NOTE: targets api.openai.com only — Responses API is OpenAI-direct; do not
// enable OPENAI_API_MODE=responses when pointing at OpenRouter / a compatible
// base URL.
import type {
  LlmMessage,
  NormalizedToolCall,
...
// KV-first key resolution (matches the Completions adapter): user override →
// Key Vault / env via resolveSecret. Dev removed the OPENAI_API_KEY env bridge
// (see openai_c.ts), so this no longer reads process.env directly. Hard-fail on
// a missing key is preserved from upstream.
...
export async function streamOpenAI(
  ...
  const key = await apiKey(apiKeys?.openai);
...
export async function completeOpenAIText(params: {
  ...
  const response = await createResponse({
    ...
    apiKey: await apiKey(params.apiKeys?.openai),
  });
