// ⚠️ OSS-3 WIP (task 2) — upstream's Responses-API OpenAI adapter, imported
// VERBATIM from willchen96/mike and NOT yet reconciled with dev's divergences.
// The backend WILL NOT BUILD until that reconciliation is done — see
// docs/prd/OSS-3/ticket.md. Our previous working Chat Completions / SDK adapter
// is preserved as ./openai_c.ts; port its divergences (Azure OpenAI support,
// KV-first secret resolution, hard-fail on missing key) onto this file, then
// delete openai_c.ts. Note: upstream already carries abortSignal plumbing, so
// adopting this file closes the "abort accepted-but-ignored" gap for free.
import type {
  LlmMessage,
  NormalizedToolCall,
