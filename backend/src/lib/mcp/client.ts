...
// Upstream divergence (sync-log: 9a1277b). Upstream's client.ts read the
// MCP-connector encryption secret directly from
...
// process.env read — that would bypass Key Vault on a security boundary.
//
// NOTE on KDF: dev keeps upstream's `scrypt(secret, "mike-user-mcp-v1", 32)`
// derivation here (this table is brand-new on dev with no pre-existing
// ciphertext to invalidate), unlike `userApiKeys.ts` which is pinned to
// sha256 for backwards compatibility with already-encrypted rows.
const ENCRYPTION_SECRET_NAME = "mcp-connectors-encryption-key";
            ...
            if (!secret) {
                throw new Error(
                    `MCP connectors encryption secret (${ENCRYPTION_SECRET_NAME}) ` +
                        ...
                        `or the env var MCP_CONNECTORS_ENCRYPTION_KEY for local dev.`,
                );
            }
 ...
 */
export function flushMcpEncryptionKey(): void {
    keyPromise = null;
}
...
async function encryptJson(value: Record<string, unknown>): Promise<{
...
export async function encryptString(value: string): Promise<{
    encrypted: string;
    iv: string;
    tag: string;
}> {
...
export async function decryptAuthConfig(
    row: ConnectorRow,
): Promise<McpConnectorAuthConfig> {
    if (
...
export async function toConnectorSummary(
    connector: ConnectorRow,
    tools: ToolCacheRow[] = [],
    oauthToken?: OAuthTokenRow | null,
    toolCount = tools.length,
): Promise<McpConnectorSummary> {
    const authConfig = await decryptAuthConfig(connector);
    return {
        id: connector.id,
        name: connector.name,
...
export async function authConfigPatch(
