...
export async function getUserMcpConnector(
    ...
    return await toConnectorSummary(
        connector,
...
export async function createUserMcpConnector(
    ...
    const auth = await authConfigPatch({
    ...
    const { data, error } = await db
        ...
        .single();
    if (error) throw error;
    return await toConnectorSummary(data as ConnectorRow);
}
...
export async function updateUserMcpConnector(
    ...
    if ("bearerToken" in input || "headers" in input) {
        ...
        const nextConfig: McpConnectorAuthConfig = current
            ? await decryptAuthConfig(current)
            : {};
        ...
        Object.assign(update, await authConfigPatch(nextConfig));
    ...
    return summary ?? (await toConnectorSummary(data as ConnectorRow));
}
...
export async function refreshUserMcpConnectorTools(
    ...
    return summary ?? (await toConnectorSummary(connector));
}
