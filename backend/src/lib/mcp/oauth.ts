...
async function storeOAuthToken(
    ...
    const row = {
        ...
        ...(await tokenSecretPatch("access_token", accessToken)),
        ...
        ...(await tokenSecretPatch("client_secret", clientSecret)),
...
export class DbMcpOAuthProvider implements OAuthClientProvider {
    ...
    async saveTokens(tokens: OAuthTokens) {
        ...
        const { error } = await this.db
            ...
            .upsert(row, { onConflict: "connector_id" });
        if (error) throw error;
        const authConfig = await decryptAuthConfig(this.connector);
        const { error: connectorError } = await this.db
            .from("user_mcp_connectors")
            .update({
                auth_type: "oauth",
                ...(await authConfigPatch({ headers: authConfig.headers })),
                updated_at: new Date().toISOString(),
            })
