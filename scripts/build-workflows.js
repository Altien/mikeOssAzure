...
const BACKEND_GENERATED_HEADER = `// This file is generated by scripts/build-workflows.js. Do not edit it directly.
// Upstream divergence (sync-log: fa21ac8): REGENERATION IS NOT SUPPORTED FROM
// THIS REPOSITORY ALONE. Canonical inputs remain in the separate
// Open-Legal-Products/mike-workflows repository, checked out locally at the
// exact sibling path ../mike-workflows/system. If that source tree is absent,
// KEEP THIS CHECKED-IN DEV OUTPUT during conflict resolution; do not invent
// missing workflow sources or regenerate from partial inputs.`;
const REGENERATION_UNSUPPORTED_MESSAGE =
  "NOT SUPPORTED in this dev fork: scripts/build-workflows.js is the retained " +
  "legacy generator and must not overwrite backend/src/lib/systemWorkflows.ts. " +
  "Canonical sources are at https://github.com/Open-Legal-Products/mike-workflows " +
  "(expected sibling path ../mike-workflows/system), but cloning them alone is " +
  "not sufficient. Preserve the checked-in dev output until the current " +
  "generator, source schema, and deferred workflow engine are adopted together.";
...
function writeGeneratedFiles(workflows) {
  ...
  fs.writeFileSync(BACKEND_OUT, backendTextWithDivergence);
...
function main() {
  // Upstream divergence (sync-log: fa21ac8): HARD STOP — dev adopted the
  // static generated workflow content but deferred upstream's corresponding
  // generator/source-schema/workflow-engine unit. KEEP THIS GUARD during
  // conflict resolution; do not make the legacy generator runnable merely
  // because ../mike-workflows exists.
  fail(REGENERATION_UNSUPPORTED_MESSAGE);
