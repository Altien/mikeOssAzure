...
// Dev's message type is named Message (mirror-era: MikeMessage).
import type { Message as MikeMessage } from "@/app/components/shared/types";
...
describe("useAssistantChat: SSE event handling", () => {
  it("captures chat_id from the stream and routes to the new chat URL", async () => {
    ...
    expect(mockGenerateTitle).toHaveBeenCalledWith("newly-minted", "first msg");
  });
  ...
  it("accumulates content_delta events into the assistant message", async () => {
    // content_delta carries INCREMENTAL fragments, not cumulative
    // state — the hook accumulates by string-concatenation into the
    // trailing content event.  A change to either side of this
    // contract would silently produce duplicated text.
    mockStreamChat.mockResolvedValue(
      sseResponse([
    ...
    expect(lastContent).toEqual({
      type: "content",
      text: "Hello, world!",
      // Dev keeps the trailing content event flagged; the flag is
      // stripped when messages are normalised into history.
      isStreaming: true,
    });
...
describe("useAssistantChat: tool / workflow events", () => {
  it("tool_call_start adds a streaming tool_call_start placeholder", async () => {
    ...
    const last = result.current.messages[result.current.messages.length - 1];
    const tool = (last.events ?? []).find((e) => e.type === "tool_call_start");
    expect(tool).toEqual({
  ...
  it("workflow_applied adds a workflow_applied event with the workflow_id + title", async () => {
    ...
    const last = result.current.messages[result.current.messages.length - 1];
    const wf = (last.events ?? []).find((e) => e.type === "workflow_applied");
    expect(wf).toEqual({
      type: "workflow_applied",
      workflow_id: "wf-1",
      title: "Summarise",
...
describe("useAssistantChat: doc_created events", () => {
  ...
  it("doc_download pushes a standalone doc_download event with the download_url", async () => {
    ...
    const last = result.current.messages[result.current.messages.length - 1];
    const download = (last.events ?? []).find((e) => e.type === "doc_download");
...
describe("useAssistantChat: doc_replicate events", () => {
  it("doc_replicate_start places a streaming doc_replicated with the count", async () => {
    ...
    const last = result.current.messages[result.current.messages.length - 1];
    const repl = (last.events ?? []).find((e) => e.type === "doc_replicated");
...
describe("useAssistantChat: streaming reveal", () => {
  it("shows the full delta text immediately while the stream is still open (dev removed the drip animation)", async () => {
    // The mirror-era hook revealed content ~8 chars per 16ms tick
    // ("drip"). Dev appends each content_delta to the visible event
    // as it arrives, flagged isStreaming until the stream settles.
    let close!: () => void;
...
`,
    ...
    const { result } = renderHook(() => useAssistantChat());
    // Kick off handleChat but don't await — the stream stays open
    // until we release `closer`.
    let chatPromise!: Promise<string | null>;
    await act(async () => {
      chatPromise = result.current.handleChat(USER_MSG("hi"));
      // Let microtasks run so the stream's first chunk is consumed.
